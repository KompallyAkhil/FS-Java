...
import java.util.*;
public class Day4P2 {
    public static int scoreOfTeam(List<Integer> t1 , List<Integer> t2 , int low ,int high , int week){
        int score = 0;
        int t1Score = 0;
        int t2Score = 0;
        for(int i : t1) t1Score += i;
        for(int i : t2) t2Score += i;
        if(t1Score < low && t2Score >= week){
            score -= 2;
        }
        else if(t1Score < low && t2Score < week){
            score -= 1;
        }
        else if(t1Score >= high && t2Score >= week){
            score += 1;
        }
        else if(t1Score >= high && t2Score < week){
            score += 2;
    ...
    public static int scoreOfTeams(int t1Score , int t2Score , int low ,int high , int week){
        int score = 0;
        if(t1Score < low && t2Score >= week){
            score -= 2;
        }
    ...
    public static void main(String[] args) {
        ...
        for(int i : nums) n1.add(i);
        for(int i : nums1) n2.add(i); 
        int i = 0;
        int score = 0;
        // while (i + size <= n) {  // Ensure window does not exceed array bounds
        //     int sco = scoreOfTeams(sum1, sum2, l, h, w);
        //     score += sco;

        //     if (i + size < n) { // Move the window forward
        //         sum1 -= nums[i];   // Remove the element going out
        //         sum2 -= nums1[i];
        //         sum1 += nums[i + size]; // Add the new element coming in
        //         sum2 += nums1[i + size];
        //     }
        //     i++;
        // }
        while (size <= nums.length) {
            ...
            // sum2 += nums1[size];
            // int sco = scoreOfTeams(sum1, sum2, l, h, w);
            int individualScore = scoreOfTeam(n1.subList(i,size), n2.subList(i,size), l, h,w);
