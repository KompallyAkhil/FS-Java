...
public class Day39P2 {
    public static int backtrack(int i , int j , int [][] grid , boolean [][] visited){
        ...
        visited[i][j] = true;
        return + 1 + backtrack(i + 1, j, grid, visited) + backtrack(i - 1, j, grid, visited) + backtrack(i, j + 1, grid, visited) + backtrack(i, j - 1, grid, visited);
    }
        ...
        for(int i = 0 ; i < n ; i++){
            for(int j = 0 ; j < m ; j++){
                if(grid[i][j] == 1 && !visited[i][j]){
                    count = Math.max(count, backtrack(i, j, grid, visited));
                }
            }
        }
